...
const SHORTCUT_LABELS={
 ...
 'rally.own':'Punkt Wir / Rallyende','rally.opponent':'Punkt Gegner / Rallyende','substitution':'Wechsel','libero':'Libero','rotation':'Rotation vor','undo':'Undo','cancel':'Aktion abbrechen'
};
const TOUR_STEPS=[
...
function pointAwardReady(){return pointAwardAllowed({quickScout:state.quickScout,matchConfigured:matchConfigured(),setReady:state.setReady,matchComplete:state.matchComplete,servingSide:state.servingSide,ownLineupReady:lineupHasPlayers('own'),opponentCapture:state.opponentCapture,opponentLineupReady:lineupHasPlayers('opponent')})}
...
function toggleLocalClock(){if(cameraAnchor||camera?.protocolConnected){setStatus('Kamera ist die aktive Zeitquelle. Aufnahme über die Kamerasteuerung stoppen.');return}state.videoClockInitialized=true;if(state.localClockRunning)stopLocalClock(true);else startLocalClock(false)}
function resetLocalClock(){if(cameraAnchor||camera?.protocolConnected){setStatus('Kamera ist die aktive Zeitquelle.');return}state.localClockRunning=false;state.localClockStartedAt=0;state.localClockElapsed=0;state.videoClockInitialized=false;state.clockMode='local';persist();render();setStatus('Lokale Zeit auf 00:00.0 zurückgesetzt.')}
...
function renderProtocol(){
 ...
 for(const e of visible){
  const tr=document.createElement('tr');const pabbr=eventPlayerAbbreviation(e),pname=eventPlayerName(e);const zone=e.action_zone?` → ${posToken(+e.action_zone)}`:'';
  tr.dataset.protocolEventId=e.id||'';tr.classList.toggle('selected',e.id===selectedProtocolEventId);tr.tabIndex=0;
  tr.innerHTML=`<td class="protocol-time-cell"><button type="button" class="protocol-time-edit" data-event-time-edit="${esc(e.id||'')}" title="${esc(t('timestamp.edit_one'))}">${esc(e.timestamp||'–')} <span aria-hidden="true">✎</span></button></td><td>${esc(e.set)}</td><td>${esc(e.rotation)}</td><td>${esc(e.player_rotation_position||e.position)}${esc(zone)}</td><td title="${esc(pname)}">${esc(pabbr)}</td><td>${esc(e.action)}</td><td>${qualityChip(e.value)}</td><td>${e.score_us}:${e.score_them}</td>`;
  const select=()=>{selectedProtocolEventId=e.id||'';renderProtocol()};tr.addEventListener('click',ev=>{if(ev.target.closest('button'))return;select()});tr.addEventListener('keydown',ev=>{if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();select()}});body.appendChild(tr)
 }
...
function clearPending(msg=true){scoutingController.resetCaptureState();if(msg)setStatus('Aktion abgebrochen.');render()}
...
async function award(who,source='',auto=false,forceSideout=false,transactionId=''){
 if(!pointAwardReady()){setStatus('Bitte zuerst den aktuellen Satz vorbereiten und das Aufschlagrecht festlegen.');return}
...
function openDrawer(view='match'){const backdrop=$('#drawerBackdrop'),body=$('#drawerBody');backdrop.hidden=false;body?.replaceChildren();body?.setAttribute('data-active-view',view);$$('#drawerNav [data-view]').forEach(b=>b.classList.toggle('selected',b.dataset.view===view));const map={match:'🏐 Spiel',players:'👥 Spielerinnen',teams:'🛡️ Teams',seasons:'📅 Saisons',rosters:'📋 Kader',data:'📄 Daten / CSV',camera:'📷 Kamera',sync:'☁️ Synchronisation',analysis:'📊 Analyse',settings:'⚙️ Einstellungen'};$('#drawerTitle').textContent=map[view]||'Menü';const draw={match:drawMatch,players:drawPlayers,teams:drawTeams,seasons:drawSeasons,rosters:drawRosters,data:drawData,camera:drawCamera,sync:drawSync,analysis:()=>analysisController.draw(),settings:drawSettings}[view];if(draw)draw();updateOverlayUiState()}
function closeDrawer(){matchEditorOpen=false;matchEditorMode='';$('#drawerBackdrop').hidden=true;updateOverlayUiState()}
function modal(title,html,onSubmit,ok='Übernehmen',onCancel=null,cancelLabel=null){$('#dialogTitle').textContent=title;$('#dialogBody').innerHTML=html;$('#dialogOk').textContent=ok;const cancelBtn=$('#dialogCancel');if(cancelBtn){cancelBtn.textContent=cancelLabel||t('common.cancel');cancelBtn.onclick=()=>closeModal()}modalSubmit=onSubmit;modalCancel=onCancel;$('#dialogBackdrop').hidden=false;updateOverlayUiState();setTimeout(()=>$('#dialogBody input, #dialogBody select')?.focus(),20)}
...
function localDeviceIcon(status=''){return `<span class="library-device-icon ${status}" title="Auf diesem Gerät" aria-label="Auf diesem Gerät"></span>`}
function libraryAvailability(m){const newer=cloudIsNewer(m),same=!!(m._cloud&&m._local&&!newer&&timestampMs(m._cloudMeta?.updatedAt)===timestampMs(m._localMeta?.updatedAt||m.updatedAt));const syncClass=newer?'sync-newer':same?'sync-equal':'sync-present';return `${m._cloud?`<span class="library-cloud-icon ${syncClass}" title="Cloud${newer?' · neuer als lokal':''}" aria-label="Cloud">☁</span>`:''}${m._local?localDeviceIcon(newer?'sync-stale':same?'sync-equal':'sync-present'):''}`}
...
async function editLibraryMatchMetadata(matchId){
 try{
  ...
  const seas=active(master.seasons),owns=active(master.teams).filter(t=>t.kind==='own'),opps=active(master.teams).filter(t=>t.kind==='opponent'),types=active(master.matchTypes);
  ...
  const body=quick
   ...
   :`<p class="small">Hier werden ausschließlich die Stammdaten dieses bestehenden Spiels korrigiert. <strong>Scoutingaktionen, Rallys, Zeitstempel, Aufstellungen und Kontext-IDs werden nicht neu erzeugt oder gelöscht.</strong></p><div class="form-grid"><label>Spieldatum</label><input name="matchDate" type="date" value="${esc(date)}" required><label>Saison</label><select name="seasonId"><option value="">–</option>${opts(seas,st.seasonId||snap.seasonId)}</select><label>Eigenes Team</label><select name="ownTeamId"><option value="">–</option>${opts(owns,st.ownTeamId||snap.ownTeamId)}</select><label>Gegner</label><select name="oppTeamId"><option value="">–</option>${opts(opps,st.oppTeamId||snap.oppTeamId)}</select><label>Spiel-/Kaderart</label><select name="matchTypeId"><option value="">–</option>${opts(types,st.matchTypeId||snap.matchTypeId)}</select></div><p class="small">Hinweis: Eine Korrektur von Team, Saison oder Spiel-/Kaderart ändert die Zuordnung des Spiels, nicht die bereits erfassten Spieleraktionen. Dadurch bleiben historische Scoutingdaten vollständig erhalten.</p>`;
  modal('Spieldaten bearbeiten',body,async fd=>{
    ...
    drawMatch();setStatus(`Spieldaten gespeichert · Spieldatum ${matchDate}.`);
    if(navigator.onLine&&['nextcloud','webdav'].includes(settings.sync?.provider)){
      try{
        ...
        await refreshCloudLibrary(false);drawMatch();setStatus(`Spieldaten gespeichert und synchronisiert · ${matchDate}.`)
      }catch(e){console.info('Spieldaten lokal gespeichert; Cloud-Sync folgt später:',e.message);setStatus(`Spieldaten lokal gespeichert · Cloud noch nicht aktualisiert: ${e.message}`)}
    }
  },'Speichern')
...
async function removeMatchFromLibrary(matchId){
 ...
 modal(t('library.remove_title'),`<p><strong>${esc(matchNames(row))}</strong></p><p class="small">${esc(t('library.remove_video_safe'))}</p><div class="library-remove-options">${choices.join('')}</div>`,fd=>{
   ...
   void withGlobalBusy(t('library.removing_cloud'),async()=>{await removeCloudMatchFromLibrary(matchId,settings.sync);removeMatchArchive(matchId);cloudMatchLibrary=cloudMatchLibrary.filter(x=>x.matchId!==matchId);setStatus(t('library.removed_sync'));drawMatch()}).catch(e=>{setStatus(e.message||String(e));alert(e.message||String(e))});
...
function drawMatch(){
 ...
 const editor=matchEditorOpen?`<div class="card match-editor-card"><div class="drawer-subhead match-editor-head"><h3>${esc(t('match.new'))}</h3><button id="cancelMatchEdit" class="ghost">Abbrechen</button></div><div class="form-grid"><label>Spieldatum</label><input id="mDate" type="date" value="${esc(formDate)}"><label>Saison</label><select id="mSeason"><option value="">–</option>${opts(seas,formSeason)}</select><label>Eigenes Team</label><select id="mOwn"><option value="">–</option>${opts(owns,formOwn)}</select><label>Gegner</label><select id="mOpp"><option value="">–</option>${opts(opps,formOpp)}</select><label>Spiel-/Kaderart</label><select id="mType"><option value="">–</option>${types.map(mt=>`<option value="${esc(mt.id)}" ${mt.id===formType?'selected':''}>${esc(mt.name)}</option>`).join('')}</select><label>Spielmodus</label><select id="mMatchMode"><option value="regular" ${formMode!=='fixed'?'selected':''}>Regulär · 3 Gewinnsätze (max. 5)</option><option value="fixed" ${formMode==='fixed'?'selected':''}>Feste Anzahl Sätze</option></select><label class="fixed-set-option">${esc(t('match.set_count'))}</label><select id="mFixedSetCount" class="fixed-set-option">${[1,2,3,4,5].map(n=>`<option value="${n}" ${+formSetCount===n?'selected':''}>${n}</option>`).join('')}</select><label class="fixed-set-option">${esc(t('match.last_set'))}</label><select id="mFixedFinalTarget" class="fixed-set-option"><option value="25" ${+formFinalTarget!==15?'selected':''}>normal bis 25</option><option value="15" ${+formFinalTarget===15?'selected':''}>verkürzt bis 15</option></select></div><div class="quality-switches"><label class="switch-row"><span>Gegner mitscouten / Startaufstellung erfassen</span><span class="switch"><input type="checkbox" id="mCaptureOpponent" ${formCapture?'checked':''}><span class="slider"></span></span></label><label class="switch-row"><span>${esc(t('match.own_detailed'))}</span><span class="switch"><input type="checkbox" id="mOwnDetailed" ${ownDetailed?'checked':''}><span class="slider"></span></span></label><small>= / - / ! / / / + / #</small><label class="switch-row"><span>${esc(t('match.opponent_detailed'))}</span><span class="switch"><input type="checkbox" id="mOppDetailed" ${oppDetailed?'checked':''}><span class="slider"></span></span></label><small>= / - / ! / / / + / #</small></div><div class="toolbar mt"><button class="primary" id="applyMatch">Spiel übernehmen</button><button id="cancelMatchEdit2">Abbrechen</button></div><p class="small">Die Spielangaben werden erst mit „Spiel übernehmen“ angelegt. „Abbrechen“ verwirft die Eingabe vollständig.</p></div>`:'';
 b.innerHTML=`${currentActions}<div class="card match-library-card"><div class="analysis-heading"><h3>Spielbibliothek <span class="preview-pill">${APP_VERSION}</span></h3><p class="small">Automatisch gespeicherte Spiele auf diesem Gerät und – bei aktiver Synchronisation – aus der Cloud.</p></div><div class="toolbar"><button id="newMatchBtn" class="primary">Neues Spiel</button><button id="refreshLibraryBtn">Cloud aktualisieren</button></div><div id="matchLibrary">${renderMatchLibrary()}</div>${editor}</div>`;
 $('#newMatchBtn').onclick=newMatchFromLibrary;$('#refreshLibraryBtn').onclick=e=>withButtonBusy(e.currentTarget,()=>refreshCloudLibrary(true),'⌛ Cloud …');wireMatchLibraryActions();refreshCloudLibrary(false);
...
function drawData(){const b=$('#drawerBody'),migrationBackups=listMigrationBackups(),latest=migrationBackups[0];b.innerHTML=`<div class="card"><h3>Scoutingdaten</h3><div class="toolbar"><button id="exportCsv" class="primary">CSV exportieren</button><button id="importCsv">CSV laden / fortsetzen</button><button id="newScout" class="danger">Scouting leeren</button></div></div><div class="card"><h3>Stammdaten</h3><div class="toolbar"><button id="exportJson">VolleyTakt-Daten exportieren</button><button id="importJson">VolleyTakt-Daten importieren</button></div><p class="small">Enthält Spielerinnen, Teams, Saisons, Saison-/Spielkader und Spiel-/Kaderarten. Cloud-Zugangsdaten werden nicht exportiert.</p></div><div class="card"><h3>Datenmigration / Sicherung</h3><p class="small">Datenschema: <strong>${CURRENT_DATA_SCHEMA}</strong>. ${latest?`Vor der letzten Schema-Migration wurde eine unveränderte Sicherung vom ${esc(new Date(latest.createdAt).toLocaleString('de-DE'))} gespeichert. Die Zuordnung alter Event-IDs zu den neuen Kontext-IDs ist darin enthalten.`:'Aktuell liegt noch kein separates Migrationsbackup vor.'}</p><div class="toolbar"><button id="downloadMigrationBackup" ${latest?'':'disabled'}>Migrationsbackup herunterladen</button></div><p class="small">Migrationsbackups werden getrennt von der Spielbibliothek gespeichert, damit sie Analyseergebnisse nicht durch ein dupliziertes Spiel verfälschen.</p></div>`;$('#exportCsv').onclick=()=>csv.download(events,`VolleyTaktLive_${new Date().toISOString().slice(0,10)}.csv`);$('#importCsv').onclick=()=>$('#csvImport').click();$('#newScout').onclick=()=>modal('Scouting dieses Spiels vollständig zurücksetzen?',`<p>Protokoll, Punkte, Sätze, Aufstellungen, Wechsel und Rallys dieses Spiels werden gelöscht.</p><p class="small">Teams, Kader und Spieleinstellungen bleiben erhalten.</p>`,()=>resetCurrentScouting(),'Scouting zurücksetzen');$('#exportJson').onclick=()=>downloadJson({schema:CURRENT_DATA_SCHEMA,master},'VolleyTakt_masterdata.json');$('#importJson').onclick=()=>$('#jsonImport').click();const mb=$('#downloadMigrationBackup');if(mb)mb.onclick=()=>{if(downloadLatestMigrationBackup())setStatus('Migrationsbackup heruntergeladen.')}}
...
function drawSync(){const c=settings.sync||{},shownUrl=normalizeServerUrl(c.url||''),b=$('#drawerBody');b.innerHTML=`<div class="card sync-card"><div class="form-grid"><label>Anbieter</label><select id="sProvider"><option value="none" ${c.provider==='none'?'selected':''}>${esc(t('sync.none'))}</option><option value="nextcloud" ${c.provider==='nextcloud'?'selected':''}>Nextcloud</option><option value="webdav" ${c.provider==='webdav'?'selected':''}>WebDAV</option><option value="google" ${c.provider==='google'?'selected':''}>Google Drive</option><option value="icloud" ${c.provider==='icloud'?'selected':''}>Apple iCloud</option><option value="onedrive" ${c.provider==='onedrive'?'selected':''}>Microsoft OneDrive</option><option value="dropbox" ${c.provider==='dropbox'?'selected':''}>Dropbox</option><option value="box" ${c.provider==='box'?'selected':''}>Box</option></select>
...
${updateCardHtml()}<div class="card about-card"><h3>About</h3><div class="about-version">VolleyTakt Live ${APP_VERSION}</div><div class="about-meta"><div>Livescouting-WebApp / PWA · ${APP_VERSION}</div><div>Lokale Datenspeicherung mit optionaler Cloud-Synchronisation</div></div><div class="about-copyright small"><strong>Copyright</strong><br>© 2026 Ingo Zech. Nutzung gemäß PolyForm Perimeter License 1.0.1.<br>${esc(t('legal.ai_notice'))}</div><div class="about-license small"><strong>Lizenzen / verwendete Medien</strong><br>Technik-Piktogramme: eigene, für VolleyTakt erstellte PNG-Piktogramme als separate App-Dateien; unabhängig austauschbar.<br>Kameraadapter: DJI-Protokollimplementierung gegen die öffentliche DJI R SDK Referenz validiert; GoPro HERO über die offizielle Open-GoPro-BLE-API.</div></div>`;
...
${section('analysis','11. Analyse','<p>Unter Analyse kannst du Zeitraum, Saison, Team, Gegner, Spielart, Spielerin, Technik, Rotation und Satz filtern. In der Analyseansicht zeigt das ⓘ-Symbol zur gewählten Analyse eine Kurzbeschreibung; zusätzlich besitzen zentrale Kennzahlen Tooltips.</p><div class="analysis-help-glossary"><h4>Spiel &amp; Phasen</h4><p><strong>K1 / Sideout:</strong> <span data-i18n-key="analysis.detail.k1">Rallys nach gegnerischem Aufschlag. Die Sideout-Quote zeigt, wie häufig das eigene Team diese Rally gewinnt und damit das Aufschlagrecht erhält.</span></p><p><strong>First-Ball-Sideout:</strong> <span data-i18n-key="analysis.detail.firstball">K1-Rallys, die bereits mit dem ersten eigenen Angriff gewonnen werden.</span></p><p><strong>K2 / Break / Breakball:</strong> <span data-i18n-key="analysis.detail.k2">Rallys bei eigenem Aufschlag. Die Break-Quote zeigt, wie häufig das eigene Team während des eigenen Aufschlags den Punkt gewinnt. Beispiel: 80 % bei 5 K2-Rallys bedeutet 4 gewonnene Rallys aus 5.</span></p><p><strong>K3 / Transition:</strong> <span data-i18n-key="analysis.detail.k3">Spielphase nach Abwehr eines gegnerischen Angriffs und anschließendem eigenem Gegenangriff. Sie wird dort ausgewertet, wo die notwendige Aktionskette vollständig erfasst ist.</span></p><p><strong>Rotations-Dashboard:</strong> <span data-i18n-key="analysis.detail.rotation">vergleicht Rallyerfolg sowie ausgewählte K1-, K2-, Annahme- und Angriffswerte nach eigener Rotation.</span></p><h4>Technik &amp; Wirkung</h4><p><strong>Aufschlagwirkung:</strong> <span data-i18n-key="analysis.detail.serve">Fehler, Asse, Zielzonen und Break-Erfolg nach eigenem Aufschlag.</span></p><p><strong>Annahme &amp; Sideout:</strong> <span data-i18n-key="analysis.detail.reception">verbindet Annahmequalität mit anschließendem Sideout- und First-Ball-Erfolg.</span></p><p><strong>Zuspielverteilung:</strong> <span data-i18n-key="analysis.detail.sets">zeigt Zuspielwege von WO nach WOHIN sowie – wenn erfasst – Tempo und Passweite/-richtung.</span></p><p><strong>Angriff Quelle → Ziel:</strong> <span data-i18n-key="analysis.detail.attacks">verknüpft Angriffszone und Zielzone mit Angriffserfolg.</span></p><p><strong>Spielerinnenwirkung:</strong> <span data-i18n-key="analysis.detail.players">fasst Aktionen und Qualitäten je Spielerin zusammen. Gewonnene Rallys mit Beteiligung sind keine Kausalitätsaussage.</span></p><h4>Vertiefende Analysen</h4><p><strong>Kontextketten:</strong> <span data-i18n-key="analysis.detail.chains">wertet vollständig erfasste Annahme → Zuspiel → Angriff-Sequenzen innerhalb derselben Rally aus.</span></p><p><strong>Gegner-Tendenzen:</strong> <span data-i18n-key="analysis.detail.opponent">zeigt gegnerische Angriffsrichtungen, Aufschlagziele und Zuspielwege, soweit sie gescoutet wurden.</span></p><p><strong>Techniken / Qualität:</strong> <span data-i18n-key="analysis.detail.techniques">zeigt Häufigkeit und Verteilung der Qualitätsstufen.</span></p><p><strong>Rallys:</strong> <span data-i18n-key="analysis.detail.rallies">listet abgeschlossene Ballwechsel mit Satz, Rotation, Aktionszahl, Gewinner und Endstand.</span></p></div><p>Die Ansichten nutzen dasselbe Datenmodell; bei einfacher Erfassung bleiben Detaildimensionen leer und werden nur dort ausgewertet, wo Daten vorhanden sind. Der Vergleichsmodus stellt Zeitraum A und B gegenüber. Analyseergebnisse öffnen in einem eigenen Ergebnisfenster, können lokal gespeichert und bei aktiver Nextcloud-/WebDAV-Synchronisation mit der Cloud abgeglichen werden. Über die Druckvorschau lassen sie sich an den Systemdruckdialog übergeben und dort auch als PDF speichern.</p>')}
...
${section('about','13. About & Lizenzen',`<p>About zeigt Version, Copyright und verwendete Medien/Lizenzen. Copyright © 2026 Ingo Zech. VolleyTakt wird gemäß PolyForm Perimeter License 1.0.1 bereitgestellt. Die Technik-Piktogramme sind eigene, für VolleyTakt erstellte App-Dateien. Der DJI-Kameraadapter verwendet das öffentliche DJI-R-SDK/BLE-Protokoll; der GoPro-Adapter nutzt die offizielle Open-GoPro-BLE-API.</p><p>${esc(t('legal.ai_notice'))}</p>`)}
...
function wire(){
 ...
 if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=0.4.1',{updateViaCache:'none'}).catch(()=>{});if(navigator.onLine)setTimeout(()=>checkForUpdate({interactive:false}),1800);
}
