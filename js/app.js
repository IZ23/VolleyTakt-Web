...
const APP_VERSION='0.4.0 RC4-r1';
const APP_VERSION_ID='0.4.0-rc4-r1';
const TECHNIQUE_ICON_REV='0.4.0-rc4-r1-pictograms';
...
const TOUR_VERSION='0.4.0-rc4-r1';
...
function newMatchFromLibrary(){
 if(state.matchId)archiveCurrentMatch(state.matchComplete?'ended':'interrupted');
 const keep={matchDate:new Date().toISOString().slice(0,10),autoRotate:state.autoRotate,allowPositionOnly:state.allowPositionOnly,opponentCapture:state.opponentCapture,ownQualityProfile:state.ownQualityProfile,opponentQualityProfile:state.opponentQualityProfile,fieldOrientation:state.fieldOrientation};
 state={...defaultState,...keep,matchId:newId('match'),videoAssignments:[]};
 events=[];redoStack=[];
 persist();
 render();drawMatch();
 setStatus('Neues Spiel angelegt. Es besitzt bereits eine eigene Spiel-ID; Spielangaben und Video können jetzt direkt zugeordnet werden.')
}
...
${updateCardHtml()}<div class="card about-card"><h3>About</h3><div class="about-version">VolleyTakt Live 0.4.0 RC4-r1</div><div class="about-meta"><div>Livescouting-WebApp / PWA · 0.4.0 RC4-r1</div><div>Lokale Datenspeicherung mit optionaler Cloud-Synchronisation</div></div><div class="about-license small"><strong>Lizenzen / verwendete Medien</strong><br>Technik-Piktogramme: eigene, für VolleyTakt erstellte PNG-Piktogramme als separate App-Dateien; unabhängig austauschbar.<br>Kameraadapter: DJI-Protokollimplementierung gegen die öffentliche DJI R SDK Referenz validiert; GoPro HERO über die offizielle Open-GoPro-BLE-API.</div></div>`;
...
function wire(){
 ...
 $('#quickScoutToggle').onchange=async()=>{
  ...
  if(toggle.checked){
   await startQuickScout({draft:true});
   return;
  }
 ...
 if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=0.4.0-rc4-r1',{updateViaCache:'none'}).catch(()=>{});if(navigator.onLine)setTimeout(()=>checkForUpdate({interactive:false}),1800);
}
