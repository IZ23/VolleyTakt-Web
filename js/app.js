...
function openDrawer(view='match'){$('#drawerBackdrop').hidden=false;$$('#drawerNav [data-view]').forEach(b=>b.classList.toggle('selected',b.dataset.view===view));const map={match:'🏐 Spiel',players:'👥 Spielerinnen',teams:'🛡️ Teams',seasons:'📅 Saisons',rosters:'📋 Kader',data:'📄 Daten / CSV',camera:'📷 Kamera',sync:'☁️ Synchronisation',analysis:'📊 Analyse',settings:'⚙️ Einstellungen'};$('#drawerTitle').textContent=map[view]||'Menü';({match:drawMatch,players:drawPlayers,teams:drawTeams,seasons:drawSeasons,rosters:drawRosters,data:drawData,camera:drawCamera,sync:drawSync,analysis:()=>analysisController.draw(),settings:drawSettings}[view]||(()=>{}))()}
...
${section('about','13. About & Lizenzen','<p>About zeigt Version, Copyright und verwendete Medien/Lizenzen. Copyright © 2026 Ingo Zech. VolleyTakt wird gemäß PolyForm Perimeter License 1.0.1 bereitgestellt. Die Technik-Piktogramme sind eigene, für VolleyTakt erstellte App-Dateien. Der DJI-Kameraadapter verwendet das öffentliche DJI-R-SDK/BLE-Protokoll; der GoPro-Adapter nutzt die offizielle Open-GoPro-BLE-API.</p>')}
...
function wire(){
 ...
 if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=0.4.0',{updateViaCache:'none'}).catch(()=>{});if(navigator.onLine)setTimeout(()=>checkForUpdate({interactive:false}),1800);
}
