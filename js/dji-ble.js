import {DJI,FrameStream,buildFrame,recordPayload,statusSubscribePayload,connectionPayload,connectionResponsePayload,parseCameraStatus,u32} from './dji-protocol.js';
...
export class DjiOsmoBle extends EventTarget{
 ...
 setStatusInterval(ms){this.statusIntervalMs=Math.max(10000,Math.min(15000,Number(ms)||12000));return this.statusIntervalMs}
 ...
 async subscribeStatus(){this.log('Abonniere DJI-Kamerastatus; Kamera liefert Status per Push, UI aktualisiert Akku im 10–15-s-Rhythmus …');await this.send(0x1D,0x05,statusSubscribePayload(),{wait:false,cmdType:0x01});}
